		...
		<div class="m-portlet__body">
			...
			<div class="m-form m-form--label-align-right m--margin-top-20 m--margin-bottom-30">
				<div class="row align-items-center">
					...
					<div class="col-xl-4 order-1 order-xl-2 m--align-right">
						<a href="{{url("/pages/create")}}" class="btn btn-accent m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
							<span>
							...
							<tr>
								...
								<td>
									<span class="m-switch m-switch--icon m-switch--success">
										<label>
											{{Form::checkbox('status',1,$item->status,array('class'=>'page-status','data-id'=>$item->id))}}
											<span></span>
										</label>
									</span>
								</td>
								<td>
									<a href="pages/{{$item->id}}/edit" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit details"><i class="la la-edit"></i></a>
									{!! Form::open(['method' => 'DELETE','class'=>'delete-form','route' => ['pages.destroy', $item->id]]) !!}
			...
			@if ($page_list->isNotEmpty())
				...
				<div class="table-information">
					<span class="info">Displaying {{$page_list->lastItem()}} of {{$page_list->total()}} records</span>
				</div>
			@endif
		</div>
	</div>
